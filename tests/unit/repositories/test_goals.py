...
from app.repositories.goals import GoalRepository, validate_goal_for_away_match, validate_goal_for_home_match, \
    validate_subsequent_goal, validate_is_last_goal
...
def away_repo_fixture(away_goal):
    repo = MagicMock()
    another_goal = Goal(match_date=datetime.now().date(), scored_by="Mark", assisted_by="Thijs", score=Score(home=0, away=2))
